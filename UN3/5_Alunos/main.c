...
/* O desafio é criar um programa em C que permita ao usuário realizar operações complexas,
como adicionar alunos, lançar notas, calcular médias e gerar relatórios detalhados para
cada turma. Para facilitar, realize de forma direta no código a inserção de alunos e a
criação de turmas (mas sem adicionar alunos às turmas). */
